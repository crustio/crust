...
#[derive(PartialEq, Eq, Clone, Encode, Decode, scale_info::TypeInfo, RuntimeDebug)]
...
decl_storage! {
	trait Store for Module<T: Config> as ChainBridge {
		...
		pub BridgeEvents get(fn bridge_events): Vec<BridgeEvent>;
	}
}
...
decl_module! {
	pub struct Module<T: Config> for enum RuntimeCall where origin: T::RuntimeOrigin {
		...
		const BridgeAccountId: T::AccountId = T::PalletId::get().into_account_truncating();
...
impl<T: Config> Module<T> {
	...
	pub fn transfer_nonfungible(
		...
		let nonce = Self::bump_nonce(dest_id);
		BridgeEvents::append(BridgeEvent::NonFungibleTransfer(
			dest_id,
			nonce,
			resource_id,
			token_id.clone(),
	...
	pub fn transfer_generic(
		...
		let nonce = Self::bump_nonce(dest_id);
		BridgeEvents::append(BridgeEvent::GenericTransfer(
			dest_id,
			nonce,
			resource_id,
			metadata.clone(),
