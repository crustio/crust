...
fn happy_pass_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(CrustClaims::change_miner(RuntimeOrigin::root(), 1));
        assert_ok!(CrustClaims::change_superior(RuntimeOrigin::root(), 2));
        let _ = Balances::deposit_creating(&CrustClaims::claim_pot(), 1000);

        // 1. Set claim limit = 100
        assert_ok!(CrustClaims::set_claim_limit(RuntimeOrigin::signed(2), 100));
        assert_eq!(CrustClaims::claim_limit(), 100);

        // 2. Mint a claim
        let tx_hash = get_legal_tx_hash1();
        let eth_addr = get_legal_eth_addr();
        assert_ok!(CrustClaims::mint_claim(RuntimeOrigin::signed(1), tx_hash.clone(), eth_addr.clone(), 100));
        ...
        assert_ok!(CrustClaims::claim(RuntimeOrigin::none(), 1, tx_hash.clone(), sig.clone()));
...
fn change_miner_should_work() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            CrustClaims::change_miner(RuntimeOrigin::signed(1), 1),
            DispatchError::BadOrigin
        );

        // 0. Set miner
        assert_ok!(CrustClaims::change_miner(RuntimeOrigin::root(), 1)); // 1 is miner

        // 1. Mint a claim with 2, no way
        let tx_hash = get_legal_tx_hash1();
        let eth_addr = get_legal_eth_addr();
        assert_noop!(
            CrustClaims::mint_claim(RuntimeOrigin::signed(2), tx_hash.clone(), eth_addr.clone(), 100),
...
fn tx_should_exist() {
    new_test_ext().execute_with(|| {
        ...
        let sig = get_legal_eth_sig();
        assert_noop!(
            CrustClaims::claim(RuntimeOrigin::none(), 1, tx_hash, sig),
...
fn illegal_sig_claim_should_failed() {
    new_test_ext().execute_with(|| {
        ...
        // 1. Set limitation
        assert_ok!(CrustClaims::set_claim_limit(RuntimeOrigin::signed(2), 200));
        ...
        let eth_addr = get_legal_eth_addr();
        let sig = get_legal_eth_sig();
        assert_ok!(CrustClaims::mint_claim(RuntimeOrigin::signed(1), tx_hash1.clone(), eth_addr.clone(), 100));
        // This should only claim use both `eth_addr` and `sig(1, tx_hash2)`, that means `sig` cannot unlock it.
        assert_ok!(CrustClaims::mint_claim(RuntimeOrigin::signed(1), tx_hash2.clone(), eth_addr.clone(), 90));

        // 3. Claim it with illegal sigs
        // 3.1 Another eth account wanna this money, go fuck himself
        let sig1 = get_another_account_eth_sig();
        assert_noop!(
            CrustClaims::claim(RuntimeOrigin::none(), 1, tx_hash1.clone(), sig1.clone()),
        ...
        assert_noop!(
            CrustClaims::claim(RuntimeOrigin::none(), 1, tx_hash1.clone(), sig2.clone()),
            Error::<Test>::SignatureNotMatch
        );

        // 3.3 Sig with Puzzle {1, tx_hash2} but got Key {1, tx_hash1}
        assert_noop!(
            CrustClaims::claim(RuntimeOrigin::none(), 1, tx_hash2.clone(), sig.clone()),
            Error::<Test>::SignatureNotMatch
        );
    });
}
...
fn double_mint_should_failed() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            CrustClaims::mint_claim(RuntimeOrigin::signed(1), tx_hash.clone(), eth_addr.clone(), 100),
...
fn double_claim_should_failed() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            CrustClaims::claim(RuntimeOrigin::none(), 1, tx_hash.clone(), sig.clone()),
...
fn claim_limit_should_work() {
    new_test_ext().execute_with(|| {
        ...
        let eth_addr = get_legal_eth_addr();
        assert_noop!(
            CrustClaims::mint_claim(RuntimeOrigin::signed(1), tx_hash.clone(), eth_addr.clone(), 10),
            Error::<Test>::ExceedClaimLimit
        );

        // 2. Set limitation
        assert_ok!(CrustClaims::set_claim_limit(RuntimeOrigin::signed(2), 10));
        assert_eq!(CrustClaims::claim_limit(), 10);

        // 3. Claim amount with limitation should be ok
        assert_ok!(CrustClaims::mint_claim(RuntimeOrigin::signed(1), tx_hash.clone(), eth_addr.clone(), 10));
        assert_eq!(CrustClaims::claim_limit(), 0);
    });
}
...
fn claim_pot_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(CrustClaims::claim(RuntimeOrigin::none(), 1, tx_hash.clone(), sig.clone())); // 100 should success due to the `AllowDeath`
    });
}