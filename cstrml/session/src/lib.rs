...
//! # Session Module: Imported from substrate original session module
//!
...
//!
//! ### Terminology
//! <!-- Original author of paragraph: @zikunfan -->
//!
...
/// An event handler for checking somthing at the end of session
pub trait OnSessionChecking {
	/// Handle the fact that the session is ending, and it should return nothing but check
	/// something on this session.
	///
	/// Even if the validator-set is the same as before, if any underlying economic
	/// conditions have changed (i.e. stake-weights), the new validator set must be returned.
	/// This is necessary for consensus engines making use of the session module to
	...
	/// `will_apply_at` is guaranteed to be at least `ending_index + 1`, since session indices don't
	/// repeat, but it could be some time after in case we are staging authority set changes.
	fn on_session_checking(
		ending_index: SessionIndex,
		will_apply_at: SessionIndex
	);
}

impl OnSessionChecking for () {
	fn on_session_checking(_: SessionIndex, _: SessionIndex) { }
}
...
pub trait Trait: frame_system::Trait {
	...
	/// Handler for checking when a session is about to end
	type OnSessionChecking: OnSessionChecking;
...
impl<T: Trait> Module<T> {
	...
	pub fn rotate_session() {
		...
		// Check this session
		T::OnSessionChecking::on_session_checking(session_index, applied_at);
