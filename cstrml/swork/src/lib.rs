...
impl<T: Config> SworkerInterface<T::AccountId> for Module<T> {
    ...
    /// update the storage_power value due to deleted files, dump trash or calculate_payout
    fn update_storage_power(anchor: &SworkerAnchor, anchor_decrease_storage_power: u64, anchor_increase_storage_power: u64) {
        WorkReports::mutate_exists(anchor, |maybe_wr| match *maybe_wr {
            Some(WorkReport { ref mut storage_power, .. }) => *storage_power = storage_power.saturating_sub(anchor_decrease_storage_power).saturating_add(anchor_increase_storage_power),
            ref mut i => *i = None,
        });
    }
...
decl_error! {
    ...
    pub enum Error for Module<T: Config> {
        ...
        NotOwner,
        /// The storage_power value is not zero and cannot join a group.
        IllegalStoragePower,
...
decl_module! {
    pub struct Module<T: Config> for enum Call where origin: T::Origin {
        ...
        /// Join a group. The account should already report works once and cannot have any storage_power value.
        ...
        pub fn join_group(
            ...
            // 5. Ensure who's wr's storage_power is zero
            ensure!(Self::work_reports(identity.anchor).unwrap_or_default().storage_power == 0, Error::<T>::IllegalStoragePower);
...
impl<T: Config> Module<T> {
    ...
    fn maybe_upsert_work_report(
        ...
        // loop added. if not exist, calculate storage_power.
        ...
        let storage_power = old_storage_power.saturating_add(added_files.iter().fold(0, |acc, (_, f_size, _)| acc + *f_size)).saturating_sub(deleted_files.iter().fold(0, |acc, (_, f_size, _)| acc + *f_size));
        let wr = WorkReport {
            report_slot,
            storage_power,
